    ...
    pub async fn start_filterchain(
        ...
        match handler {
            ConnectionHandler::Http(http_connection_manager) => {
                with_metric!(http::DOWNSTREAM_CX_TOTAL, add, 1, shard_id, &[KeyValue::new("listener", listener_name.to_string())]);
                with_metric!(http::DOWNSTREAM_CX_ACTIVE, add, 1, shard_id, &[KeyValue::new("listener", listener_name.to_string())]);
                defer! {
                ...
                let (stream, selected_codec) = if let Some(tls_configurator) = tls_configurator {
                    let (stream, negotiated) =
                        start_tls(&http_connection_manager.listener_name, stream, tls_configurator, Some(codec_type))
                            .await?;
                    with_metric!(tls::HANDSHAKES, add, 1, shard_id, &[KeyValue::new("listener", listener_name.to_string())]);
            ...
            ConnectionHandler::Tcp(tcp_proxy) => {
                with_metric!(tcp::DOWNSTREAM_CX_TOTAL, add, 1, shard_id, &[KeyValue::new("listener", listener_name.to_string())]);
                with_metric!(tcp::DOWNSTREAM_CX_ACTIVE, add, 1, shard_id, &[KeyValue::new("listener", listener_name.to_string())]);
                defer! {
                    with_metric!(tcp::DOWNSTREAM_CX_DESTROY, add, 1, shard_id, &[KeyValue::new("listener", listener_name.to_string())]);
                    with_metric!(tcp::DOWNSTREAM_CX_ACTIVE, sub, 1, shard_id, &[KeyValue::new("listener", listener_name.to_string())]);
                    let ms = u64::try_from(start_instant.elapsed().as_millis()).unwrap_or(u64::MAX);
                    with_histogram!(tcp::DOWNSTREAM_CX_LENGTH_MS, record, ms, &[KeyValue::new("listener", listener_name.to_string())]);
                }
