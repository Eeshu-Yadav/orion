...
use orion_configuration::config::cluster::{
    ...
    InternalEndpointAddress, LbEndpoint as LbEndpointConfig, LbPolicy, LocalityLbEndpoints as LocalityLbEndpointsConfig,
};
use tracing::debug;
...
impl Ord for LbEndpoint {
    fn cmp(&self, other: &Self) -> std::cmp::Ordering {
        match (&self.address, &other.address) {
            (EndpointAddressType::Socket(a), EndpointAddressType::Socket(b)) => {
...
impl LbEndpoint {
    pub fn grpc_service(&self) -> Result<GrpcService> {
        match &self.connection {
            EndpointConnection::Socket { http_channel, .. } => {
                match &self.address {
                    EndpointAddressType::Socket(authority) => {
                        GrpcService::try_new(http_channel.clone(), authority.clone())
                    }
                    EndpointAddressType::Internal(_) => {
                        Err("Internal endpoints don't support gRPC service".into())
                    }
                }
            }
            EndpointConnection::Internal(_) => {
                Err("Internal endpoints don't support gRPC service yet".into())
            }
        }
    }
    ...
    pub fn build(self) -> Result<Arc<LbEndpoint>> {
        ...
        let connection = match &address {
            EndpointAddressType::Socket(authority) => {
                ...
                let tcp_channel = TcpChannelConnector::new(authority, cluster_name, bind_device.clone(), self.connect_timeout, self.transport_socket.clone());
                EndpointConnection::Socket { http_channel, tcp_channel }
            }
            EndpointAddressType::Internal(internal_addr) => {
                EndpointConnection::Internal(InternalConnection {
...
impl TryFrom<LbEndpointConfig> for PartialLbEndpoint {
    ...
    fn try_from(lb_endpoint: LbEndpointConfig) -> Result<Self> {
        ...
        let address = match lb_endpoint.address {
            ...
            EndpointAddress::Internal(internal_addr) => {
                EndpointAddressType::Internal(internal_addr)
            }
        };
    ...
    impl LbEndpoint {
        ...
        pub fn new(
            ...
            let http_channel = HttpChannelBuilder::new(bind_device.clone())
                ...
                .build()
                .unwrap();
            let tcp_channel = TcpChannelConnector::new(&authority, cluster_name, bind_device.clone(), None, UpstreamTransportSocketConfigurator::None);
