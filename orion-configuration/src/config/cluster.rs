...
fn simplify_lb_endpoints<S: Serializer>(value: &Vec<LbEndpoint>, serializer: S) -> Result<S::Ok, S::Error> {
    if value.iter().all(|s| is_default(&s.health_status) && s.load_balancing_weight == NonZeroU32::MIN) {
        ...
        let socket_addresses: Vec<SocketAddr> = value.iter()
...
mod envoy_conversions {
    #![allow(deprecated)]
        ...
        InternalEndpointAddress, InternalUpstreamTransport, LbEndpoint, LbPolicy, LocalityLbEndpoints,
        MetadataKind, MetadataValueSource, OriginalDstConfig, OriginalDstRoutingMethod, TlsConfig, TlsSecret, TransportSocket,
    };
    use crate::config::{
        common::*,
        core::Address,
    ...
    use orion_data_plane_api::envoy_data_plane_api::{
        envoy::{
            ...
            r#type::metadata::v3::{
                ...
                metadata_key::path_segment::Segment,
            },
        },
        google::protobuf::Any,
        prost::Message,
            ...
            let address = match required!(host_identifier)? {
                EnvoyHostIdentifier::Endpoint(EnvoyEndpoint {
                    ...
                    match address {
                        ...
                        Address::Pipe(_, _) => Err(GenericError::unsupported_variant("Pipe addresses are not supported for endpoints")),
                    }
                })(),
    ...
    impl TryFrom<EnvoyInternalUpstreamTransport> for InternalUpstreamTransport {
        ...
        fn try_from(value: EnvoyInternalUpstreamTransport) -> Result<Self, Self::Error> {
            ...
            let passthrough_metadata = passthrough_metadata
                .into_iter()
                .map(MetadataValueSource::try_from)
