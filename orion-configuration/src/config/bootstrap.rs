...
mod envoy_conversions {
    #![allow(deprecated)]
    use super::{Admin, Bootstrap, BootstrapExtension, DynamicResources, InternalListenerBootstrap, Node, StaticResources};
    ...
    impl TryFrom<EnvoyBootstrap> for Bootstrap {
        ...
        fn try_from(envoy: EnvoyBootstrap) -> Result<Self, Self::Error> {
            ...
            Ok(Self { static_resources, node, dynamic_resources, admin, stats_flush_interval, stats_sinks, bootstrap_extensions })
        }
    }
    ...
    impl TryFrom<EnvoyAdmin> for Admin {
        ...
        fn try_from(envoy: EnvoyAdmin) -> Result<Self, Self::Error> {
                ...
                .ok_or(GenericError::MissingField("address is mandatory to setup admin interface"))?
            {
                ...
                _ => {
                    return Err(GenericError::UnsupportedVariant(std::borrow::Cow::Borrowed("Only SocketAddress is supported")));
                },
            };
    ...
    impl TryFrom<EnvoyTypedExtensionConfig> for BootstrapExtension {
        ...
        fn try_from(value: EnvoyTypedExtensionConfig) -> Result<Self, Self::Error> {
            ...
            match name.as_str() {
                "internal_listener" => {
                    if typed_config.type_url == "type.googleapis.com/envoy.extensions.bootstrap.internal_listener.v3.InternalListener" {
                        ...
                        Err(GenericError::from_msg(format!("Unsupported bootstrap extension type: {}", typed_config.type_url)))
                    }
                }
