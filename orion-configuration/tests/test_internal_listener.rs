use orion_configuration::config::{
    ...
    cluster::{
        EndpointAddress, InternalEndpointAddress, InternalUpstreamTransport, LbEndpoint, 
        MetadataKind, MetadataValueSource, TransportSocket,
    },
...
fn test_internal_listener_serialization() {
    let internal_listener = InternalListener {
        buffer_size_kb: Some(1024),
    };
...
fn test_bootstrap_extension_serialization() {
    let internal_listener_bootstrap = InternalListenerBootstrap {
        buffer_size_kb: Some(2048),
    };
